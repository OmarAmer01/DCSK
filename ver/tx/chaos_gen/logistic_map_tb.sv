...
module logistic_map_tb;
  ...
  task automatic report_fail(int i, int test_no, ref int err_ctr);
    err_ctr++;
    $error("[FAIL] STIM = %2h => Found %4h Expected %4h", i_seed, o_next_val,
           test_no == 1 ? test_1_csv[i] : test_2_csv[i]);
  endtask
...
*/
    ...
    //* Print Test Summary
    $display("\n\n");
    $display("[TEST 1] %0d Failed (%0d Passed) Out of 256 Cases.", err_cnt_tst_1,
             256 - err_cnt_tst_1);

    $display("[TEST 2] %0d Failed (%0d Passed) Out of 256 Cases.", err_cnt_tst_2,
             256 - err_cnt_tst_2);
